...
def compute_option_score(option, profile: str):
    ...
    risk = option["risk"]
    risk_factor = 1 - risk if profile == "safe" else risk if profile == "aggressive" else 1 - abs(risk - 0.5)
...
def predict_wild_west(profile): return _predict(profile, "wild_west", [
...
def predict_kamikaze(profile): return _predict(profile, "kamikaze", [
...
def predict_dragons_gold(profile): return _predict(profile, "dragons_gold", [
...
def predict_royal_feast(profile): return _predict(profile, "royal_feast", [
...
def predict_game(game: str, profile: str):
    ...
    return {"game": game, "best_path": None, "survival_chance": None, "predicted_score": None, "reason": "Je ne connais pas encore ce jeu, mais je peux apprendre.", "profile": profile}
...
def log_game(event_type: str, game: str, profile: str, prediction: dict):
    ...
    if len(game_logs) > 200: game_logs.pop(0)
...
async def poe_handler(request: Request):
    ...
    return JSONResponse(content={"message": "Commande inconnue. Utilise par exemple : 'start
